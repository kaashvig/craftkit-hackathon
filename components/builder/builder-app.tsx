...
export default function BuilderApp() {
  const [state, setState] = useState<BuilderState>({ nodes: [], selectedId: null, preview: false })
  const [appName, setAppName] = useState<string>("Builder")

  // load persisted state + name
  useEffect(() => {
    try {
      const saved = localStorage.getItem(LS_KEYS.state)
      if (saved) setState(JSON.parse(saved))
      const savedName = localStorage.getItem(LS_KEYS.appName)
  ...
  return (
    <main className="min-h-[calc(100dvh-64px)] w-full" style={{ backgroundColor: COLORS.navy }}>
      <Topbar
        appName={appName}
        preview={!!state.preview}
        onTogglePreview={() => setState((s) => ({ ...s, preview: !s.preview }))}
        onSave={() => {
          try {
            localStorage.setItem(LS_KEYS.state, JSON.stringify(state))
          } catch {}
        ...
        onClear={() => {
          setState({ nodes: [], selectedId: null, preview: false })
        }}
      />
      <section className="grid grid-cols-1 md:grid-cols-[256px_minmax(0,1fr)_304px] gap-4 p-4 md:p-6">
        <Palette />
        <Canvas
          nodes={state.nodes}
          preview={!!state.preview}
          onDropType={(type) => {
            ...
            setState((s) => ({ ...s, nodes: [...s.nodes, { id, type, props }], selectedId: id }))
          }}
          onSelect={(id) => setState((s) => ({ ...s, selectedId: id }))}
          onUpdate={(id, nextProps) =>
            setState((s) => ({
              ...s,
              nodes: s.nodes.map((n) => (n.id === id ? { ...n, props: { ...(n.props || {}), ...nextProps } } : n)),
            }))
          }
        ...
        <PropertiesPanel
          ...
          onChange={(next) => {
            ...
            setState((s) => ({
              ...s,
              nodes: s.nodes.map((n) => (n.id === id ? { ...n, props: { ...(n.props || {}), ...next } } : n)),
            }))
          }}
        />
      </section>
